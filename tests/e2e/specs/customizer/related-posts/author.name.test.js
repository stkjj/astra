...
describe( 'Related Posts correct Author Name', () => {
	it( 'Related Posts should display correct author name', async () => {
		...
		await createUser( { username : 'teamAstra01' } );
		await createUser( { username : 'teamAstra02' } );
		await createUser( { username : 'teamAstra03' } );
		await createUser( { username : 'teamAstra04' } );

		await page.goto( createURL( '/wp-admin/users.php' ), {
